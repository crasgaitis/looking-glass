...
def compute_band_powers(eegdata, index, timestamp, fs):
    ...
    """
    # 1. Compute the PSD
    winSampleLength, _ = eegdata.shape
    ...
    delta_power = PSD[f < 4, :]
    theta_power = PSD[(f >= 4) & (f <= 8), :]
    alpha_power = PSD[(f >= 8) & (f <= 12), :]
    beta_power = PSD[(f >= 12) & (f < 30), :]

    # create a series with appropriate column names and values
    column_names = [
        str(index) + 'delta_' + str(i) for i in range(1, delta_power.shape[0] + 1)
    ] + [
        str(index) + 'theta_' + str(i) for i in range(1, theta_power.shape[0] + 1)
    ] + [
        str(index) + 'alpha_' + str(i) for i in range(1, alpha_power.shape[0] + 1)
    ] + [
        str(index) + 'beta_' + str(i) for i in range(1, beta_power.shape[0] + 1)
    ] + [
        str (index) + 'timestamp'
    ]
        
    row_values = np.concatenate([delta_power.ravel(), theta_power.ravel(), alpha_power.ravel(), beta_power.ravel(), np.array([np.mean(timestamp)])])
    result_series = pd.Series(row_values, index=column_names)
